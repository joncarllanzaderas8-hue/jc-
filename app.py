...
location_mode = st.sidebar.selectbox("Select Monitoring Site", ["A - Green Space (TUP CAVITE) ", "B - Residential (PALIPARAN III JOLIBEE)", "C - Commercial (SM PALA-PALA)"])
...
with st.expander("🪪 Model Card"):
    ...
    st.write("*Training window*: Update here with your training start & end timestamps.")
    if hasattr(model, 'classes_'):
        st.write(f"*Classes*: {list(model.classes_)}")
    else:
        st.write("*Classes*: Not available from model object.")
    st.write("*Evaluation*: Add accuracy/F1 from your training notebook; include cross-validation details.")

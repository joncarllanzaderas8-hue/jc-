...
# PAGE CONFIG
# ─────────────────────────────────────────────
st.set_page_config(page_title="Arduino Sensor Forecast Dashboard", layout="wide")

# ─────────────────────────────────────────────
# SIGNAL CONFIGURATION
# ─────────────────────────────────────────────
SIGNALS = {
    'tempC':    {'label': 'Temperature', 'unit': '°C', 'warn': 35},
    'humidity': {'label': 'Humidity', 'unit': '%', 'warn': 90},
    'mqRaw':    {'label': 'MQ Gas', 'unit': 'raw', 'warn': 350},
    'aqi':      {'label': 'AQI', 'unit': '', 'warn': 50},
}
...
def holt_forecast(series, alpha=HOLT_ALPHA, beta=HOLT_BETA, steps=FORECAST_STEPS):
    ...
    if n < 2:
        return np.full(steps, y[-1]), np.full(steps, y[-1]), np.full(steps, y[-1]), 0
...
def parse_serial_line(line):
    ...
    if line.startswith("{"):
        try:
            d = json.loads(line)
            if all(k in d for k in SIGNALS):
                return clean_incoming(d)
        except:
            pass
    ...
    if len(parts) >= 4:
        try:
            ...
            d = {keys[i]: float(parts[i]) for i in range(4)}
            return clean_incoming(d)
        except:
            pass
...
# ─────────────────────────────────────────────
# SIMULATION / SERIAL THREAD
# ─────────────────────────────────────────────
def serial_reader_thread(live_buffer):

    last = {k: live_buffer[-1][k] if live_buffer else 28 for k in SIGNALS}

    noise = {'tempC':0.3,'humidity':0.8,'mqRaw':8,'aqi':1.5}
    trend = {'tempC':0.02,'humidity':-0.05,'mqRaw':0.5,'aqi':0.1}

    while not stop_event.is_set():

        for k in SIGNALS:
            last[k] += trend[k] + np.random.normal(0, noise[k])

        row = clean_incoming(last.copy())
...
def build_chart(df, column):
    ...
    fc, lo, hi, rmse = holt_forecast(ser.values)
...
for ts,row in df_history.iterrows():
    live_buffer.append({"timestamp":ts,**row.to_dict()})
...
# DASHBOARD HEADER
# ─────────────────────────────────────────────
st.title("Arduino Environmental Monitoring Dashboard")
...
while True:
    ...
    with placeholder.container():
        ...
        for i,(k,cfg) in enumerate(SIGNALS.items()):
            val = df_live[k].iloc[-1]
            cols[i].metric(cfg['label'], f"{val:.2f} {cfg['unit']}")

        st.divider()

        chart_cols = st.columns(2)

        chart_cols[0].plotly_chart(build_chart(df_live,'tempC'), use_container_width=True)
        chart_cols[1].plotly_chart(build_chart(df_live,'humidity'), use_container_width=True)

        chart_cols = st.columns(2)

        chart_cols[0].plotly_chart(build_chart(df_live,'mqRaw'), use_container_width=True)
        chart_cols[1].plotly_chart(build_chart(df_live,'aqi'), use_container_width=True)
